...
class customAdam3(torch.optim.Optimizer):
    def __init__(self, params, lr = 1e-3, betas = (0.99,0.999,0.9999), eps = 1e-3):
        defaults = dict(lr = lr, betas = betas, eps = eps)
        super(customAdam3,self).__init__(params,defaults)

    def __setstate__(self, state) -> None:
        return super(customAdam3).__setstate__(state)
    ...
    def step(self,closure = None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                s_cap = (state['exp_avg']/(bias_correction1))
                r_cap = (state['exp_avg_sq']/(bias_correction2)).sqrt()
                t_cap = (state['exp_avg_third']/(bias_correction3))**(1/3)

                s_cap[s_cap != s_cap] = 1e-9
                r_cap[r_cap != r_cap] = 1e-9
                ...
                p.data.addcdiv_(s_cap, r_cap + t_cap + group['eps'], value=-group['lr'])
